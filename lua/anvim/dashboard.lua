...
function M.open()
  local ok, err = pcall(function()
    ...
    vim.keymap.set("n", "j", function() M.nav(1) end, { buffer = buf, nowait = true, silent = true, desc = "Next" })
    vim.keymap.set("n", "k", function() M.nav(-1) end, { buffer = buf, nowait = true, silent = true, desc = "Previous" })
    vim.keymap.set("n", "<Down>", function() M.nav(1) end, { buffer = buf, nowait = true, silent = true, desc = "Next" })
    vim.keymap.set("n", "<Up>", function() M.nav(-1) end, { buffer = buf, nowait = true, silent = true, desc = "Previous" })
    vim.keymap.set("n", "<CR>", function() M.select() end, { buffer = buf, nowait = true, silent = true, desc = "Select" })
    vim.keymap.set("n", "q", function() M.close() end, { buffer = buf, nowait = true, silent = true, desc = "Close" })
    vim.keymap.set("n", "<Esc>", function() M.close() end, { buffer = buf, nowait = true, silent = true, desc = "Close" })
    vim.keymap.set("n", "c", function() M.close(); require("anvim.help_check").interactive() end, { buffer = buf, nowait = true, silent = true, desc = "Check system" })
    vim.keymap.set("n", "r", function() M.close(); local p = require("anvim.project").detect(); require("anvim.tasks").run(p, "run") end, { buffer = buf, nowait = true, silent = true, desc = "Run app" })
    vim.keymap.set("n", "l", function() M.close(); require("anvim.logcat").open() end, { buffer = buf, nowait = true, silent = true, desc = "Logcat" })

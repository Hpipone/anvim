...
function M.setup(opts)
  ...
  register_cmd("AnvimTaskClose", function() require("anvim.tasks").close_output() end, "Close anvim task output window")
  register_cmd("AnvimTaskSave", function(opts)
    local path = (opts and opts.args ~= "" and opts.args) or nil
    require("anvim.tasks").save(path)
  end, "Save last task output to file", { nargs = "?" })
